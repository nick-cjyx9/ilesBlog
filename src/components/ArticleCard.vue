...
<template>
  <div v-if="post.frontmatter.cover" class="w-full h-full max-w-4xl pb-10">
    <a :href="post.href" class="block relative w-full h-full hover:shadow-xl">
      <div class="absolute top-0 left-0 w-full h-full z-10 bg-opacity-10 bg-gray-500">
      ...
      <div class="cover px-6 nl:px-10 pb-10 pt-32 relative z-20 
        ...
        <time class="text-sm font-semibold">{{ formatTime(post.date) }}</time>
