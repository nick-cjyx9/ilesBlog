...
<template>
  <div v-if="post.frontmatter.cover" class="w-full block mb-8 ">
    <a :href="post.href" class="block relative w-full h-full">
      <div class="absolute top-0 left-0 w-full h-full z-10">
        <div
          :style="`background-image:url(${useCoverUrl(post.frontmatter.cover)})`"
          class="bg-no-repeat bg-cover bg-center w-full h-full"
        />
      </div>
      <div
        class="cover px-6 nl:px-10 pb-10 pt-32 relative z-20
  ...
  <div v-else class="w-full mb-8 dark:text-white shadow dark:shadow-none">
    <a :href="post.href" class="block relative w-full h-full">
