<script setup lang="ts">
// import { reactive, onMounted } from 'vue'
const props = defineProps(['content']);
// import EasyTyper from 'easy-typer-js'
// const obj = reactive({
//     output: '',
//     isEnd: false,
//     speed: 25,
//     singleBack: true,
//     sleep: 0,
//     type: 'normal',
//     backSpeed: 40,
//     sentencePause: true
// })
// onMounted(() => {
//     const typing = new EasyTyper(obj, props.content, () => { }, () => { })
// })
</script>
<template>
    <div class="rounded-lg mx-10 mt-6 p-5 w-full border-gray-300 border-2">
        <HeadTitle class="!text-lg">AI 生成的摘要</HeadTitle>
        <!-- <span class="w-full text-gray-600 dark:text-gray-300" style="font-size: 14.5px;">{{ obj.output }}</span> -->
        <span class="w-full text-gray-600 dark:text-gray-300" style="font-size: 14.5px;">{{ content }}</span>
    </div>
</template>

