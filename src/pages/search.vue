    ...
    <div class="holder w-full min-h-[500px] p-8">
        <form class="w-full mx-auto" id="searchForm">   
            ...
            <div class="relative">
                ...
                <input type="search" id="searchBox" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="输入关键词搜索博文" required />
        ...
        <div class="flex flex-col text-important p-8">
            ...
            <small class="text-minor w-full text-end mt-3">Build with <a href="https://www.fusejs.io/" class="text-link">fuse.js</a> - 高级语法参见<a href="https://www.fusejs.io/api/query.html" class="text-link">此处</a></small>
        </div>
    </div>
    <!-- tailwind -->
    <div class="hidden px-4 break-all h-[100px] overflow-hidden text-ellipsis">${processedContent}</div>
    <li class="hidden w-full text-minor-link px-3 py-1" role="button"><a href="/post/${consult['item']['pathName'].replace('.mdx','')}" class="w-full">${counter} - <span class="underline">${processedTitle}</span></a></li>
</template>
<script client:load>
...
document.getElementById('searchForm').addEventListener('submit',(event)=>{
    ...
    let consults = fuse.search(searchPattern)
    ...
    consults.forEach(consult => {
        ...
        consult['matches'].forEach((match)=>{
            ...
            if (match['key']=='content'){ for(let indice of match['indices'].reverse())
