...
const remarkBiliLink: Plugin<[], Root> = function () {
  ...
  return (ast) => {
    ...
    visit(ast, 'text', (node, _, parent) => {
      const matches = [...node.value.matchAll(syntaxPattern)]
      if (matches.length !== 0) {
        let l = 0
        const output = []
        matches.forEach((match) => {
          if (l !== match.index)
            output.push({ type: 'text', value: node.value.slice(l, match.index) })
          l = match.index + match[0].length
          // 变形为 html 节点
          output.push({
            type: 'mdxFlowExpression',
            value: `<iframe src="//player.bilibili.com/player.html?bvid=${match[1]}&as_wide=1&high_quality=1&amp;page=1" frameborder="no" scrolling="no" width="95%" height="600"></iframe>`,
